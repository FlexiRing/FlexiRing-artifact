...
void RealtimeSinPredict() {
  static float x_value = 0.0f;
  const float x_step = 0.1f; 
  

  const tflite::Model* model = ::tflite::GetModel(g_hello_world_float_model_data);
  static HelloWorldOpResolver local_op_resolver;
  static bool is_initialized = false;
  

  alignas(16) static uint8_t local_tensor_arena[3000];
  static tflite::MicroInterpreter interpreter(model, local_op_resolver, local_tensor_arena, 3000);

  if (!is_initialized) {
    local_op_resolver.AddFullyConnected();
  ...
  MicroPrintf("%f, %f", x_value, y_pred);

 
  x_value += x_step;
  if (x_value > 2 * 3.1415926f) {
    x_value = 0.0f;
  }
}
}  // namespace
